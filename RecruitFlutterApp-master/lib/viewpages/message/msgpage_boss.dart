...
import 'message_resume.dart';



class MsgPageBoss extends StatefulWidget {



  @override
  _MsgPageBossState createState() => _MsgPageBossState();
}

class _MsgPageBossState extends State<MsgPageBoss> with SingleTickerProviderStateMixin {
...
class _CompanyBodyListState extends State<CompanyBodyList> with AutomaticKeepAliveClientMixin{
  ...
  _OnRefresh(){
     //id，分为boss，用户
    new PinPinReponse().getMessageList(SharepreferUtils.getBosss().userId,0).then((value) {
          ...
          if (data.length >0 ) {
            return GestureDetector(
              onTap: (){
                ...
                String reply_id ;
                if(userId == SharepreferUtils.getBosss().userId){
                  reply_id = data[index]["reply_id"].toString();
                }else{
                  reply_id = userId;
                }
                Navigator.push(context,
                    MaterialPageRoute(builder: (context) => PageChatLine(user_id:SharepreferUtils.getBosss().userId ,reply_id: reply_id,head_icon:data[index]["userInfo"]["head_img"],title: data[index]["userInfo"]["user_name"],type: 0,)));
              },
              ...
              switch(model.link){
                case "感兴趣":
                  Navigator.push(
                      context,
                      MaterialPageRoute(
                        builder: (context) => MessageResume(0),
                      ));
                  break;
                case "看过我的":
                  Navigator.push(
                      context,
                      MaterialPageRoute(
                        builder: (context) => MessageResume(1),
                      ));
                  break;
                case "智能推荐":
                  Navigator.push(
                      context,
                      MaterialPageRoute(
                        builder: (context) => MessageResume(2),
                      ));
                  break;
                case "职位上新":
                  Navigator.push(
                      context,
                      MaterialPageRoute(
                        builder: (context) => MessageResume(3),
                      ));
                  break;
              }
            },
