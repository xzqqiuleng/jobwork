...
import 'page_compdetail.dart';
import 'itemrow_compy.dart';


class CompJobListp extends StatefulWidget {
  final bool contentScrollable;
  final ValueChanged<int> tabbarItemClick;



  CompJobListp({Key key,this.contentScrollable,this.tabbarItemClick}) : super(key:key);

  @override
  _CompanyJobState createState() => _CompanyJobState();
}

class _CompanyJobState extends State<CompJobListp> with SingleTickerProviderStateMixin {
...
class _CompanyBodyListState extends State<CompanyBodyList> with AutomaticKeepAliveClientMixin{
  ...
  _OnRefresh(){
    ...
    new PinPinReponse().getCompanyList(page,searchType:widget.type).then((value) {
            ...
            return  GestureDetector(
              ...
              onTap: () {
                Navigator.push(
                    context,
                    MaterialPageRoute(
                      builder: (context) => PageCompDetail( data[index]["id"]),
                    ));
              },
            );
          }
