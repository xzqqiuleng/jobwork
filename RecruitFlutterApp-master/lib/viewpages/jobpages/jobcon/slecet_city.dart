...
import 'model_citydata.dart';


class SelectCCitys extends StatefulWidget {
  final double height;
  final Color  themeColor;
  final String locationIcon;
  final ValueChanged<String> onValueChanged;

  SelectCCitys({Key key,this.height,this.locationIcon = '', this.themeColor = Colors.grey,this.onValueChanged}): super(key:key);

  @override
  _SelectCCitysState createState() => _SelectCCitysState();
}

class _SelectCCitysState extends State<SelectCCitys> {
 List<ModelCityData> models = [];
 int currentSelect = 0;
 bool isTap = false;
 ...
 Future<void> fetchData() async {
    try {
      ...
      ModelCityData hotModel = ModelCityData(name: "常用",currentCity: "广州",citys: hotCitys,historyCitys: historyCitys);
      models.add(hotModel);

      moduleData.forEach((data) {
        models.add(ModelCityData.fromJson(data));
      });
...
String proviceName="";
  Widget _buildProvincesItem(ModelCityData model,int index) {
  ...
  Widget _buildNormalCitys() {
    final double width = (ScreenUtils.width * 0.75 - 15.0 * 4) / 3;
